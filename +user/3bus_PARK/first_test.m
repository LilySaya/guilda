...
%{
idx = 1;
 Q  = [1e-5, 1e3, 1e-5, 1e-5, 1e-5, 1e-5, 1e-5];          
 R  = 1e-9;
retrofit_hinf1 = controller_retrofit_LQR(net,idx,diag(Q),diag(R));
%retrofit_hinf1 = controller_retrofit_hinf(net,idx);
net.add_controller_local(retrofit_hinf1);
idx = 2;
retrofit_hinf2 = controller_retrofit_LQR(net,idx,diag(Q),diag(R));
...
%fault
option = struct();
option.fault = {{[0 0.01], 1}, {[20 20.01], 1},{[40 40.01], 1}};
...
plot(out_1axis.t,[out_1axis.X{1}(:,2)]);
hold on;
plot(out_2axis.t,[out_2axis.X{1}(:,2)]);
hold on;
plot(out_park.t,[out_park.X{1}(:,2)]);
legend('1axis','2axis','park');
%}