...
%         I_equilibrium
    ...
    methods
        ...
        function x_st = set_equilibrium(obj, V, I)
            ...
            syms del 
            eqn = Xqp*Iabs*cos(del-Iangle)+(Xq-Xqp)*Iabs*sin(del-Iangle)-Vabs*sin(del-Vangle) == 0;
            S = double(solve(eqn));
            if (abs(S(1))<abs(S(2))); delta = real(S(1)); else; delta = real(S(2)); end
            
            %delta = Vangle + atan(P/(Q+Vabs^2/Xq));
            Vq = Vabs*cos(delta-Vangle); %Vabs*cos(delta-Vang)
            Vd = Vabs*sin(delta-Vangle); %Vabs*sin(delta-Vang)
            Iq = Iabs*cos(delta-Iangle);
            Id = Iabs*sin(delta-Iangle);
            Ed = (Xq-Xqp)*Iabs*sin(delta-Iangle);
            Eq = (Id*Xdp)+Vq;
