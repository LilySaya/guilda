...
% 　出力　：componentクラスのインスタンス
    ...
    methods
        ...
        function [dx, con] = get_dx_constraint(obj, t, x, V, I, u)
            ...
            %x_gen includes variables delta, omega, E
            x_gen = x(1:nx);
            x_avr = x(nx+(1:nx_avr));
            x_pss = x(nx+nx_avr+(1:nx_pss));
            x_gov = x(nx+nx_avr+nx_pss+(1:nx_gov));
            
            %V(1) real V, V(2) imag V
            Vabs = norm(V);
            ...
            Vabscos = V(1)*cos(delta)+V(2)*sin(delta);%Vabs*cos(delta-Vang)
            Vabssin = V(1)*sin(delta)-V(2)*cos(delta);%Vabs*sin(delta-Vang)
            ...
            [dx_pss, vpss] = obj.pss.get_u(x_pss, omega);
            [dx_avr, Vfd] = obj.avr.get_Vfd(x_avr, Vabs, Efd, u(1)-vpss);
        ...
        function [A, B, C, D, BV, DV, BI, DI, R, S] = get_linear_matrix(obj, x_st, Vst)
            ...
            %Efd differentiated by [delta, E, Vreal, Vimag]
            dEfd = -[dVabscos, 0, dVabscos_dV] * (Xd/Xdp-1) + [0, Xd/Xdp, 0, 0];
            
            %Ir =  (E-Vabscos)*sin(delta)/Xdp + Vabssin*cos(delta)/Xq;
            %Ii = -(E-Vabscos)*cos(delta)/Xdp + Vabssin*sin(delta)/Xq;
            %Ireal, Iimag differentiated by delta
            ...
            %}
            syms s_delta s_Eq s_Vreal s_Vimag
            ...
            P = [s_Vreal, s_Vimag]*I; %2

            Efd = (Xd/Xdp)*s_Eq - (Xd-Xdp)*Vq/Xdp ;%3

            J = jacobian([P, Efd, I'], ...
                      [s_delta, s_Eq, s_Vreal, s_Vimag]);
            sys_fb = ss(eval(subs(J,[s_delta, s_Eq, s_Vreal, s_Vimag],[delta, E, Vst(1), Vst(2)])));
        ...
        function x_st = set_equilibrium(obj, V, I)
            ...
            eq = [eq1;eq2];
            S = solve(eq);
            if(S.e(1)>0);delta = double(S.del(1));E = double(S.e(1));
            else; delta = double(S.del(2)); E = double(S.e(2));end
            delta
            E
            %to here
            %}
